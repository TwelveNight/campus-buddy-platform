...
public class GroupController {
    ...
    @GetMapping
    public R<Map<String, Object>> getGroups(
        ...
        // 查询所有小组的创建者信息
        List<Long> creatorIds = groupList.stream().map(Group::getCreatorId).distinct().collect(Collectors.toList());
        Map<Long, User> creatorMap = new HashMap<>();
        if (!creatorIds.isEmpty()) {
            userService.listByIds(creatorIds).forEach(u -> creatorMap.put(u.getUserId(), u));
        }
        // 查询当前用户在这些小组的成员状态
        User currentUser = getCurrentUser();
            ...
            List<GroupMember> userMembers = groupMemberService.list(
                    new LambdaQueryWrapper<GroupMember>()
                            .in(GroupMember::getGroupId,
                                    groupList.stream().map(Group::getGroupId).collect(Collectors.toList()))
                            .eq(GroupMember::getUserId, currentUser.getUserId()));
            userMembers.forEach(m -> userStatusMap.put(m.getGroupId(), m.getStatus()));
        }
        // 封装返回结构：每个group加上creator和memberStatus字段
        List<Map<String, Object>> records = groupList.stream().map(g -> {
            ...
            // 当前用户在该小组的成员状态
            map.put("memberStatus", userStatusMap.getOrDefault(g.getGroupId(), null));
            return map;
